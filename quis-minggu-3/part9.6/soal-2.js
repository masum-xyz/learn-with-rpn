...
function shoppingTime(memberId, money) {
    ...
    let belanjaan = [];
    let produk = {
        nama: ['Sepatu Stacattu', 'Baju Zoro', 'Baju H&N', 'Sweater Uniklooh', 'Casing Handphone'],
    ...
    } else if (money < produk.harga[produk.harga.length - 1]) {
        return 'Mohon maaf, uang tidak cukup';
    }

    // pembelian barang
    for (let i =0; i < produk.harga.length; i++) {
        if (uang >= produk.harga[i]) {
            uang -= produk.harga[i];
            belanjaan.push(produk.nama[i]);
        } 
    }
