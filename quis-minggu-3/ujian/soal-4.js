...
function graduates (students) {
  ...
  for (let i = 0; i < students.length; i++) {
    if (students[i].score > 75) {
      let classKey = students[i].class;
      if (result[classKey] === undefined) {
        result[classKey] = [{name: students[i].name, score: students[i].score}];
      } else {
        result[classKey].push({name: students[i].name, score: students[i].score});
      }
    }
  }
  return result;
